...
const Favorite = () => {
  ...
  const handleAddAllToCart = () => {
    favorites.forEach((product) => addToCart(product, 1));
  };
  ...
  return (
    <div className="w-full h-[calc(100vh-82px)] pt-6 md:p-8 gap-6 flex flex-col">
      <div className="flex items-center justify-center md:justify-between">
        ...
        <Button
          className="h-16 bg-[#53B175] w-64 hidden md:flex text-white text-lg font-semibold rounded-2xl disabled:opacity-50"
      ...
      <div className="relative md:static w-full h-full flex items-center justify-center px-6 md:px-0 gap-6">
        ...
        <div className="w-full absolute bottom-0 left-0 p-6 md:hidden bg-white flex items-center justify-center gap-4">
          <Button
            className="h-16 bg-[#53B175] w-full max-w-md text-white text-lg font-semibold rounded-2xl disabled:opacity-50"
            onClick={handleAddAllToCart}
            disabled={favorites.length === 0}
          >
            Add All To Cart
